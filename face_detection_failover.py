...
dsfd = face_detection.build_detector(
  "DSFDDetector", confidence_threshold=.65, nms_iou_threshold=.3)
yoloface = YoloDetector(yolo_type='yolov5l', target_size=720, gpu=0)
...
def face_detection_failover(rgb_img:np.array)->Dict[str, Any]:
    ...
    for degree in [0,1,3,2]:
        ...
        yolo_bboxes, yolo_confs, yolo_points = yoloface.predict(img_rot, conf_thres = 0.65)
        ...
        try:
            for degree in [0,1,3,2]:
                img_rot = np.rot90(rgb_img, k=degree)
                dsfd_res = dsfd.detect(img_rot)
                if len(dsfd_res) != 0:                   
                    bbox_idx = filter_bbox(img_rot, dsfd_res)
